	...
	$.fn.srt = function (options) {
		...
		$(this).on('play', function () {
			...
			play_subtitles = setInterval(function () {
				let current_subtitle = undefined;
				let currentTime = $obj[0].currentTime;

				var subtitle = subtitles.find(x => x.st < currentTime && x.et >= currentTime);

				if (subtitle) {
					var subtitleIndex = subtitles.indexOf(subtitle);
					//console.log('Found subtitle -->', subtitle);
					if (current_subtitle || subtitleIndex != current_subtitle) {
						current_subtitle = subtitleIndex;
